...
[lmcrec-query](PlaybackToolsCatalog.md#lmcrec-query) will run queries against
...
- [lmcrec-info](PlaybackToolsCatalog.md#lmcrec-info) to gather time span
  information ([sample output](./sample-output/lmcrec-info/output.txt))
- [lmcrec-inventory](PlaybackToolsCatalog.md#lmcrec-inventory) to gather
...
`RECORD_FILES_DIR`](PlaybackToolsCatalog.md#inst-v-record_files_dir) and
[`auto` Output Dir](PlaybackToolsCatalog.md#auto-output-dir).
...
[lmcrec-report](PlaybackToolsCatalog.md#lmcrec-report) command.
...
[lmcrec-inventory](PlaybackToolsCatalog.md#lmcrec-inventory) generates the
...
[lmcrec-merge-schema](PlaybackToolsCatalog.md#lmcrec-merge-schema) is the tool
...
[lmcrec-inventory](PlaybackToolsCatalog.md#lmcrec-inventory) for the entire
...
set, max size, etc). [lmcrec-export](PlaybackToolsCatalog.md#lmcrec-export)
...
The [lmcrec-export](PlaybackToolsCatalog.md#lmcrec-export) will create the following subdirectory and file structure under the output dir:
...
1. Export the data with [lmcrec-export](PlaybackToolsCatalog.md#lmcrec-export):
...
[lmcrec-cleanup](PlaybackToolsCatalog.md#lmcrec-cleanup) will read the config
...
driven events. [lmcrec-stats](PlaybackToolsCatalog.md#lmcrec-stats) (see
...
[lmcrec-dump](./PlaybackToolsCatalog.md#lmcrec-dump) can be used to display
record, info and index files.

Note that the Python decoder maps all value carrying records into a
pseudo-record type `VAR_VALUE`; the original record type as per the [Data
Model](./DataModel.md) is returned in `file_record_type` field.

```text
LmcRecord(record_type=<LmcrecType.VAR_VALUE: 17>, var_id=17, value=9, file_record_type=<LmcrecType.VAR_UINT_VAL: 7>)
```
